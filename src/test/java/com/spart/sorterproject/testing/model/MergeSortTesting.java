...
public class MergeSortTesting {
    ...
    @Test
    public void mergeSortOutputTestNormal(){
        ...
        assertTrue(mergeSortTest.mergeSort(testArrNormal,1).equals("[4, 10, 30, 40, 90, 100, 900]"));
    }
    // tests if an array with two values that are the same
    @Test
    public void mergeSortOutputTestDuplicateValue(){
        MergeSort mergeSortTest = new MergeSort();
        int [] testArrNormal = {10,10,900,4,30,90,100};
        String arrTestString = Arrays.toString(testArrNormal);
        assertTrue(mergeSortTest.mergeSort(testArrNormal,1).equals("[4, 10, 10, 30, 90, 100, 900]"));
    }

    // tests if an array with all values the same
    @Test
    public void mergeSortOutputTestAllDuplicate(){
        MergeSort mergeSortTest = new MergeSort();
        int [] testArrNormal = {10,10,10,10,10,10,10};
        String arrTestString = Arrays.toString(testArrNormal);
        assertTrue(mergeSortTest.mergeSort(testArrNormal,1).equals("[10, 10, 10, 10, 10, 10, 10]"));
    }

    // tests an array of negative values with one positive
    @Test
    public void mergeSortOutputTestNegative(){
        MergeSort mergeSortTest = new MergeSort();
        int [] testArrNormal = {-10,-106,-3,-200,-2,-78,10};
        String arrTestString = Arrays.toString(testArrNormal);
        assertTrue(mergeSortTest.mergeSort(testArrNormal,1).equals("[-200, -106, -78, -10, -3, -2, 10]"));
    }
    //tests an empty array
    @Test
    public void mergeSortOutputTestEmpty(){
        MergeSort mergeSortTest = new MergeSort();
        int [] testArrNormal = {};
        String arrTestString = Arrays.toString(testArrNormal);
        assertTrue(mergeSortTest.mergeSort(testArrNormal,1)==null);
    }
    //Tests a very large array of numbers
    @Test
    public void mergeSortOutputTestLargeArray(){
        MergeSort mergeSortTest = new MergeSort();
