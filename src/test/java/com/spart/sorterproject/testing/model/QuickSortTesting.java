...
public class QuickSortTesting {
    ...
    @Test
    public void quickSortOutputTestNormal(){
        ...
        assertTrue(quickSortTest.quickSort(testArrNormal).equals("[4, 10, 30, 40, 90, 100, 900]"));
    }
    // tests if an array with two values that are the same
    @Test
    public void quickSortOutputTestDuplicateValue(){
        QuickSort quickSortTest = new QuickSort();
        int [] testArrNormal = {10,10,900,4,30,90,100};
        String arrTestString = Arrays.toString(testArrNormal);
        assertTrue(quickSortTest.quickSort(testArrNormal).equals("[4, 10, 10, 30, 90, 100, 900]"));
    }

    // tests if an array with all values the same
    @Test
    public void quickSortOutputTestAllDuplicate(){
        QuickSort quickSortTest = new QuickSort();
        int [] testArrNormal = {10,10,10,10,10,10,10};
        String arrTestString = Arrays.toString(testArrNormal);
        assertTrue(quickSortTest.quickSort(testArrNormal).equals("[10, 10, 10, 10, 10, 10, 10]"));
    }

    // tests an array of negative values with one positive
    @Test
    public void quickSortOutputTestNegative(){
        QuickSort quickSortTest = new QuickSort();
        int [] testArrNormal = {-10,-106,-3,-200,-2,-78,10};
        String arrTestString = Arrays.toString(testArrNormal);
        assertTrue(quickSortTest.quickSort(testArrNormal).equals("[-200, -106, -78, -10, -3, -2, 10]"));
    }
    //tests an empty array
    @Test
    public void quickSortOutputTestEmpty(){
        QuickSort quickSortTest = new QuickSort();
        int [] testArrNormal = {};
        String arrTestString = Arrays.toString(testArrNormal);
        assertTrue(quickSortTest.quickSort(testArrNormal)==null);
    }
    //Tests a very large array of numbers
    @Test
    public void quickSortOutputTestLargeArray(){
        QuickSort quickSortTest = new QuickSort();
