...
import com.sparta.sorterproject.model.ArraySort;
...
public class ArraySortTesting {
    ...
    @Test
    public void arraySortOutputTestNormal(){
        ArraySort arraySortTest = new ArraySort();
        int [] testArrNormal = {10,40,900,4,30,90,100};
        String arrTestString = Arrays.toString(testArrNormal);
        assertTrue(arraySortTest.arraysSort(testArrNormal).equals("[4, 10, 30, 40, 90, 100, 900]"));
    }
    // tests if an array with two values that are the same
    @Test
    public void arraySortOutputTestDuplicateValue(){
        ArraySort arraySortTest = new ArraySort();
        int [] testArrNormal = {10,10,900,4,30,90,100};
        String arrTestString = Arrays.toString(testArrNormal);
        assertTrue(arraySortTest.arraysSort(testArrNormal).equals("[4, 10, 10, 30, 90, 100, 900]"));
    }

    // tests if an array with all values the same
    @Test
    public void arraySortOutputTestAllDuplicate(){
        ArraySort arraySortTest = new ArraySort();
        int [] testArrNormal = {10,10,10,10,10,10,10};
        String arrTestString = Arrays.toString(testArrNormal);
        assertTrue(arraySortTest.arraysSort(testArrNormal).equals("[10, 10, 10, 10, 10, 10, 10]"));
    }

    // tests an array of negative values with one positive
    @Test
    public void arraySortOutputTestNegative(){
        ArraySort arraySortTest = new ArraySort();
        int [] testArrNormal = {-10,-106,-3,-200,-2,-78,10};
        String arrTestString = Arrays.toString(testArrNormal);
        assertTrue(arraySortTest.arraysSort(testArrNormal).equals("[-200, -106, -78, -10, -3, -2, 10]"));
    }
    //tests an empty array
    @Test
    public void arraySortOutputTestEmpty(){
        ArraySort arraySortTest = new ArraySort();
        int [] testArrNormal = {};
        String arrTestString = Arrays.toString(testArrNormal);
        assertTrue(arraySortTest.arraysSort(testArrNormal).equals("[]"));
    }
    //Tests a very large array of numbers
    @Test
    public void arraySortOutputTestLargeArray(){
        ArraySort arraySortTest = new ArraySort();
